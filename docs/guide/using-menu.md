...
```php
use unlite\admin\components\MenuHelper;
...
Return of `unlite\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `unlite\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `unlite\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
```php
<?php
user unlite\admin\components\Helper;
