...
- Enh: Custom side menu via `unlite\admin\Module::items` (mdmunir).
...
- Chg: Using `VarDumper::export` to save `data` of `unlite\models\AuthItem` (mdmunir).
...
- Chg: Can customize return of `unlite\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `unlite\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `unlite\admin\Module::allowActions`. Set access control directly with `unlite\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `unlite\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
